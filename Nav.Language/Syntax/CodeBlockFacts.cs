...
/// <summary>
/// Der „Host" eines Code-Blocks (<c>[ … ]</c>) — er bestimmt, welche Code-Schlüsselwörter dort grammatisch
/// zulässig sind. Die zugehörigen Grammatikregeln stehen im <c>NavParser</c> (jeweils die optionalen
/// <c>code*</c>-Deklarationen des Hosts); die erlaubten Schlüsselwörter je Host liefert
...
static class CodeBlockFacts {
    ...
    /// <c>null</c>, wenn er keiner der Host-Deklarationsknoten ist. Die <em>einzige</em> Autorität für die
    /// Zuordnung Knotentyp → Host: sowohl die Completion (Host eines gerade getippten <c>[</c>, siehe
    ...
    /// eigenen Knotentyp — er ist die <em>Abwesenheit</em> eines umschließenden Host-Knotens.
    /// </summary>
    ...
    /// <summary>
    /// Die im Host zulässigen Code-Deklarations-Schlüsselwörter in <b>Grammatik-Reihenfolge</b> — spiegelt
    ...
    /// <summary>
    /// Die im Host zulässigen, <b>sichtbaren</b> Code-Deklarations-Schlüsselwörter — die versteckten
    ...
    /// <summary>
    /// Ob eine Code-Deklaration im selben Host <b>mehrfach</b> zulässig ist. Die Grammatik erlaubt jede
    ...
    /// <summary>
    /// Die im Host noch <b>anbietbaren</b>, sichtbaren Code-Deklarations-Schlüsselwörter:
    /// <see cref="VisibleDeclarationKeywords"/> ohne die am selben Host bereits vorhandenen Singletons.
    /// Wiederholbare Deklarationen (<c>using</c>, siehe <see cref="IsRepeatable"/>) bleiben stets erhalten.
    /// <paramref name="presentKeywords"/> sind die am Host schon deklarierten Code-Schlüsselwörter.
    /// </summary>
