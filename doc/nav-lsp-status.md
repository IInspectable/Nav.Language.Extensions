...
> **Nächste Session-Aufgabe:** **Folding** (`textDocument/foldingRange` ↔ `OutliningTagger`) oder
> **Completion** (`textDocument/completion` ↔ `AsyncCompletionSource`) für den LSP-Server.
> **Hover** (`textDocument/hover`), References/documentHighlight und GoTo Definition (Option B)
> sind **erledigt** — siehe unten.
...
- ~~**Hover** (`textDocument/hover` ↔ `SymbolQuickInfoSource`)~~ — **erledigt.** Engine-Kern
  ...
  (klassifizierte `DisplayParts` aus dem vorhandenen `DisplayPartsBuilder` + Symbol-`Location`). Caret-Auflösung
  über `SymbolPosition.SymbolsAt` (spezifischstes Symbol mit nicht-leeren Anzeige-Teilen; `init`-mit-Alias wird
  wie in der VS-QuickInfo übersprungen). Server: `textDocument/hover`-Handler + `HoverProvider`-Capability;
  rendert die Signatur als ` ```nav `-Codeblock (`MarkupContent`, Markdown) mit Symbol-Range. WPF-spezifische
  QuickInfo-Erweiterungen (Choice/EdgeMode-Aufruflisten in `QuickinfoBuilderService.Visitor`) bleiben VS-only.
  PoC braucht **keine** Client-Änderung (vscode-languageclient fragt Hover bei advertiseter Capability automatisch).
  Tests: `Nav.Language.Tests/QuickInfo/NavHoverServiceTests.cs` (5) grün net472+net10 (Suite 638).
