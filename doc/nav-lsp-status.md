...
> `didChange`/`didClose` ist **erledigt** (siehe unten); als Nächstes `workspace/didChangeWatchedFiles`
> (externe Änderungen geschlossener Dateien — schließt zugleich die Disk-Edit-Lücke des Include-Graphen),
> inkrementeller Doc-Sync, MessagePack-Pin. Alternativ der **Phase-3-Spike** (VS als RPC-Client). Die Completion ist vollständig
...
- **Server starten (framework-dependent):** `dotnet Nav.Language.Server\bin\Debug\net10.0\nav.lsp.dll`
  (.NET-10-Runtime ist installiert). **AssemblyName ist `nav.lsp`** (nicht mehr `Pharmatechnik.Nav.Language.Server`);
  `RootNamespace` bleibt `Pharmatechnik.Nav.Language.Server`, die Namespaces im Code sind unverändert.
- **Self-contained Server publizieren:** `Publish-Lsp.bat` → `deploy\lsp\nav.lsp.exe` — **genau eine Datei**
...
- ~~**Dependency-aware Re-Diagnose:**~~ **erledigt** (NOCH NICHT committet). Bei `didChange`/`didClose` werden
...
- ~~**Self-contained Publish:**~~ **erledigt.** `Publish-Lsp.bat` (Repo-Root, analog `Build.bat` via `vswhere`)
  → `deploy\lsp\nav.lsp.exe` als **Single-File** (`win-x64`, **genau 1 Datei**, ~39 MB komprimiert, inkl. .NET-Runtime).
  Kern: `MSBuild.exe …Nav.Language.Server.csproj -restore -t:Publish -p:RuntimeIdentifier=win-x64 -p:SelfContained=true`
  ...
  automatisch (s. u.). Extension paketieren: `npx @vscode/vsce package`.
