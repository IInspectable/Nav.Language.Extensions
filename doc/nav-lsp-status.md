...
> (siehe unten); als Nächstes inkrementeller Doc-Sync und der MessagePack-Pin (GHSA-hv8m-jj95-wg3x). Alternativ
> der **Phase-3-Spike** (VS als RPC-Client). Die Completion ist vollständig
...
- ~~**`workspace/didChangeWatchedFiles`**~~ (externe Änderungen, auch geschlossener Dateien) **erledigt**
  (NOCH NICHT committet). Client meldet *.nav-Datei-Events über `synchronize.fileEvents`
