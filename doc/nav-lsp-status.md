...
> **Nächste Session-Aufgabe:** **Code Actions** (`textDocument/codeAction`) für den LSP-Server — die
> übrige Refactoring-/CodeFix-Infrastruktur in `Nav.Language/CodeFixes/` ist bereits VS-frei und wartet
> nur auf einen Server-Handler (analog zum jetzt fertigen Rename). Die Completion ist vollständig
...
- **Code Actions** (`*SuggestedAction`/`CodeFix`).
