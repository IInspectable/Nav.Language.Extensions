...
public sealed class NavGrammarGenerator: IIncrementalGenerator {
    ...
    static void Emit(SourceProductionContext spc, ImmutableArray<GrammarRule> rules) {
        ...
        sb.AppendLine();
        sb.AppendLine("namespace Pharmatechnik.Nav.Language;");
        sb.AppendLine();
        sb.AppendLine("public static partial class NavGrammar {");
        sb.AppendLine();
        sb.AppendLine("    /// <summary>Die vollständige, aus den <c>Parse*</c>-EBNF-Fragmenten zusammengesetzte Grammatik der Nav-Sprache.</summary>");
        sb.Append("    public const string Ebnf = ");
        sb.Append(ToVerbatim(grammar));
        sb.AppendLine(";");
        sb.AppendLine();
        sb.AppendLine("    /// <summary>Die einzelnen Produktionen, je Nichtterminal-Name (linke Seite) das zugehörige EBNF-Fragment.</summary>");
        sb.AppendLine("    public static IReadOnlyDictionary<string, string> Rules { get; } = new Dictionary<string, string> {");
        foreach (var rule in ordered) {
            sb.Append("        { ");
            sb.Append(ToVerbatim(rule.RuleName));
            sb.Append(", ");
            sb.Append(ToVerbatim(rule.Ebnf));
            sb.AppendLine(" },");
        }
